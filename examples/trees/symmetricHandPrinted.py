...
thumb0Joint = PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2, SE3.Trans(3*multiplier,0,0)@SE3.Rx(np.pi/2), screwRadius)
thumb0 = tree.addJoint(palm, thumb0Joint,
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

thumb1 = tree.addJoint(thumb0, PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2,
            SE3.Trans(4*multiplier,0,0)@SE3.Rx(np.pi/4), screwRadius),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)
...
pointer1 = tree.addJoint(0, PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2,
            SE3.Trans(0,0,4*multiplier)@SE3.Ry(-np.pi/2)@SE3.Rx(np.pi), screwRadius),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

pointer2 = tree.addJoint(pointer1, PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2,
                        SE3.Trans(5*multiplier,0,0), screwRadius),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)
...
middleJoint = PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2, SE3.Trans(-2*multiplier,0,1)@SE3.Ry(0)@SE3.Rx(-np.pi/2)@SE3.Rz(np.pi), screwRadius)
middle0 = tree.addJoint(0, middleJoint,
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

middle1Joint = PrintedOrthogonalRevoluteJoint(r, -np.pi/2, np.pi/2, SE3.Trans(3*multiplier,0,0)@SE3.Rx(np.pi/2), screwRadius)
...
#     tree.setJointState(i, 0)
#     tree.setJointState(middle1, -np.pi/2)
#     tree.setJointState(middle2, -np.pi/4)
#     tree.setJointState(thumb0, -np.pi/2)
#     tree.setJointState(thumb1, -np.pi/4)
#     tree.setJointState(pointer1, np.pi/2)
#     tree.setJointState(pointer2, np.pi/2.5)
