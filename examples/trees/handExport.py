...
r = 1     #in inches - currently not working with r != 1, needs debugging
numSides = 4
screwHoleRadius = 0.1 #in inches for 2mm diameter screw with some tolerance
...
palmJoint = ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength, SE3.Trans(4*displacementScale,-1*displacementScale,1.5*displacementScale)@SE3.Ry(0)@SE3.Rx(np.pi/5.5) @ SE3.Rz(-np.pi/6))
palm = tree.addJoint(0, palmJoint,
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

thumb0Joint = ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength, SE3.Trans(jointLength*displacementScale,0,0)@SE3.Rx(1.95*np.pi/3))
thumb0 = tree.addJoint(palm, thumb0Joint,
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

thumb1 = tree.addJoint(thumb0, ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength,
                                           SE3.Trans(5*displacementScale,0,0)),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

thumbEnd = tree.addJoint(thumb1, EndTip(numSides, r, 
        SE3.Trans(3.75*displacementScale,0,0)@SE3.Ry(np.pi/2)@SE3.Rz(np.pi/2), 1),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)


pointer1 = tree.addJoint(0, ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength,
            SE3.Trans(0,0,8*displacementScale)@SE3.Ry(-np.pi/2)@SE3.Rx(np.pi)),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

pointer2 = tree.addJoint(pointer1, ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength,
                        SE3.Trans(4*displacementScale,0,0)),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

pointerEnd = tree.addJoint(pointer2, EndTip(numSides, r, 
        SE3.Trans(4*displacementScale,0,0)@SE3.Ry(np.pi/2)@SE3.Rz(np.pi/2), 1),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

middle0 = tree.addJoint(0, ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength,
        SE3.Trans(-2.25*displacementScale,0,4*displacementScale)@SE3.Ry(-np.pi/2)@SE3.Rx(np.pi/2)),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)

middle1 = tree.addJoint(middle0, ExtendedRevoluteJoint(numSides, r, np.pi, extensionLength,
                        SE3.Trans(4*displacementScale,0,0)@SE3.Rx(np.pi/2)),
              relative=True, safe=False, 
              fixedPosition=True, fixedOrientation=True)
...
printedTree.export3DKinematicTree("printedHandForExport")
