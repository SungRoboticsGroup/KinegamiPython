...
def robotFromChain(chain):
    arr = []
    qlim = [[],[]]
    angles = chain.Joints[0].Pose.eul()
    arr.append(rtb.ET.Rz(angles[0]))
    arr.append(rtb.ET.Ry(angles[1]))
    arr.append(rtb.ET.Rz(angles[2]))
    for i in range(0,len(chain.Joints)):
        pose = chain.Joints[i].Pose
        if (isinstance(chain.Joints[i], RevoluteJoint)):
            arr.append(rtb.ET.Rz())
            qlim[0].append(-np.pi)
            qlim[1].append(np.pi)
        elif (isinstance(chain.Joints[i], PrismaticJoint)):
            arr.append(rtb.ET.tz())
            qlim[0].append(0)
            qlim[1].append(2)

        if (i < len(chain.Joints) - 1):
            pose2 =  chain.Joints[i + 1].Pose
...
def robotFromJointList(joints):
    arr = []
    qlim = [[],[]]
    angles = joints[0].Pose.eul()
    arr.append(rtb.ET.Rz(angles[0]))
    arr.append(rtb.ET.Ry(angles[1]))
    arr.append(rtb.ET.Rz(angles[2]))
    for i in range(0,len(joints)):
        pose = joints[i].Pose
        if (isinstance(joints[i], RevoluteJoint)):
            arr.append(rtb.ET.Rz())
            qlim[0].append(-np.pi)
            qlim[1].append(np.pi)
        elif (isinstance(joints[i], PrismaticJoint)):
            arr.append(rtb.ET.tz())
            qlim[0].append(0)
            qlim[1].append(2)
        
        if (i < len(joints) - 1):
            pose2 =  joints[i + 1].Pose
            transform = pose.inv() * pose2
            angles = transform.eul()
            arr.append(rtb.ET.tx(transform.t[0]))
            arr.append(rtb.ET.ty(transform.t[1]))
